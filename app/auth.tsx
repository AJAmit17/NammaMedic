...
export default function AuthScreen() {
  ...
  const authenticate = async () => {
    try {
      ...
      let promptMessage = "Authenticate to access Namma Medic";
      let fallbackLabel = Platform.OS === 'ios' ? "Use Passcode" : "Use PIN";

      if (hasHardware && hasBiometrics) {
        if (Platform.OS === 'android') {
          if (supportedTypes.includes(AuthenticationType.FINGERPRINT)) {
            promptMessage = "Use Fingerprint to access Namma Medic";
          } else {
            promptMessage = "Use biometric authentication to access Namma Medic";
          }
        } else if (Platform.OS === 'ios') {
          if (supportedTypes.includes(AuthenticationType.FACIAL_RECOGNITION)) {
            promptMessage = "Use Face ID to access Namma Medic";
          } else if (supportedTypes.includes(AuthenticationType.FINGERPRINT)) {
            promptMessage = "Use Touch ID to access Namma Medic";
          }
        }
      }
